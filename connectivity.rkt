...
;;
;; I Joshua Nielson submit this assignment as my own work
#lang racket
...
(define/contract (build-init-graph input)
  (-> input-format? graph?)
  (define (h hsh commlist)
    (match commlist
      ['() hsh]
      [`((node ,n) . ,rst)  (h (hash-set hsh n (set n)) rst)]
      [`((link ,n1 ,n2) . ,rst) (h (hash-set
                                       hsh
                                       n1
                                       (set-add (hash-ref hsh n1) n2)) rst)]))
  (h (hash) input))
  ;; TODO TODO TODO
   ;(hash-set hsh somevar (set-add (hash-ref hsh somevar) othervar))]) ;;I Think that the end part is right
    ;;I just have to figure out how to input the data
    ;;The recursion isn't added yet =
...
(define (forward-link? graph n0 n1)
  ...
  (set-member? (hash-ref graph n0) n1))
...
(define (add-link graph from to)
  (hash-set graph from (set-add (hash-ref graph from) to)))
...
(define (one-step-transitive-closure graph)
  ;; For all n0 in Node(graph)
  (foldl (λ (n0 graph)         ;;We first got the list of keys and have taken 1 key and the graph are holding it
           (foldl (λ (n1 graph)             ;;This is the start of the function of the first lambda, we still have the graph and now we have the set of nodes connected with n0
                    (foldl (λ (n2 graph)   ;;n1 will be one elm from this set, so now we just have to add n2 to n0, n2 being all the nodes n1 is connected to
                             (add-link graph n0 n2));;This is where the function goes to add n0 to n2
                             graph
                             (set->list (hash-ref graph n1))))      ;;Need to give a list from a set gotcha gotcha
                   ;; For all the n2 s.t. there's an edge n1 n2 in Edges (graph:
                   ;; Add n0 n2 to Edges (graph) (addEdge and hash-ref)
                   
                 graph
                 ;; all of the n1s (called range indexing),give us a set which we can tranform to a list
...
(define (transitive-closure graph)
  (if (equal? (one-step-transitive-closure graph) graph)
      graph ;; all done, at a fixed point, graph is transitively closed
      (transitive-closure (one-step-transitive-closure graph) ))) ;; keep going around

(define testhash (hash "n0" (set "n3" "n5" "n0" "n1") "n1" (set "n3" "n0" "n1" "n2") "n2" (set "n3" "n4" "n5" "n6" "n2") "n3" (set "n3" "n0" "n1" "n2") "n4" (set "n4" "n6" "n7") "n5" (set "n5") "n6" (set "n4" "n6" "n7") "n7" (set "n7" "n0")))
;;
